...
class Norm4Test extends munit.FunSuite {
  ...
  test("A norm4 has an Int with the information of the amount required for being in that norm, for each stars and victories. Also its number"){
    assertEquals(norma.Stars_obj,70)
    assertEquals(norma.Victories_obj,6)
    assertEquals(norma.NumberNorm,4)
  }
  test("We can upgrade a Norm4 to a Norm5 of a character"){
    ...
    player.addVictories(11) /*player has the requirement ok*/
    ...
    player.addStars(121) /*player has the requirement ok*/
    player.CurrentNorm.upgradeNorm(player)
    assert(player.CurrentNorm.isInstanceOf[Norm5])
  }
}
