...
class Norm2Test extends munit.FunSuite {
  ...
  test("A norm2 has an Int with the information of the amount required for being in that norm, for each stars and victories. Also its number"){
    assertEquals(norma.Stars_obj,10)
    assertEquals(norma.Victories_obj,1)
    assertEquals(norma.NumberNorm,2)
  }
  test("We can upgrade a Norm2 to a Norm3 of a character"){
    ...
    player.addVictories( 3) /*player has the requirement ok*/
    ...
    player.addStars(30) /*player has the requirement ok*/
    player.CurrentNorm.upgradeNorm(player)
    assert(player.CurrentNorm.isInstanceOf[Norm3])
  }
}
