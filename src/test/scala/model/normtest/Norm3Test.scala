...
class Norm3Test extends munit.FunSuite {
  ...
  test("A norm3 has an Int with the information of the amount required for being in that norm, for each stars and victories. Also its number"){
    assertEquals(norma.Stars_obj,30)
    assertEquals(norma.Victories_obj,3)
    assertEquals(norma.NumberNorm,3)
  }
  test("We can upgrade a Norm2 to a Norm3 of a character"){
    ...
    player.addVictories(7) /*player has the requirement ok*/
    ...
    player.addStars(75)   /*player has the requirement ok*/
    player.CurrentNorm.upgradeNorm(player)
    assert(player.CurrentNorm.isInstanceOf[Norm4])
  }
}
