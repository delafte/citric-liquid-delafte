...
import model.Panels.PanelTypesClasses._
...
class BoardTest extends FunSuite {
  ...
  */
  ...
  test("A Board has panels connected to each other in the order of the input, also, the fist one connects with the last and the second, the last one with the first and the previous") {
    ...
    assertEquals(GamePanels(0).nextPanels, ArrayBuffer[Panel](GamePanels(5), GamePanels(1)))
    assertEquals(GamePanels(3).nextPanels, ArrayBuffer[Panel](GamePanels(2),GamePanels(4)))
